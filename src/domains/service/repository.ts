    ...
    try {
      ...
      const { data, error } = await serverMutate.insert({
        ...
        payload: {
          ...
          title: tkt.title || (tkt as any).issueType || 'Power Asset Service Ticket',
          description: tkt.description || (tkt as any).diagnosisNotes || 'Service Ticket Dispatch',
          ticket_type: tkt.ticketType || (tkt as any).issueType || 'Breakdown',
          priority: tkt.priority || 'Medium',
          status: tkt.status || 'Assigned',
          source: tkt.source || 'Portal',
          assigned_to: tkt.assignedTo || (tkt as any).assignedEngineerId,
        },
        select: '*, customers(company_name), equipment(model, serial_number)'
      });
